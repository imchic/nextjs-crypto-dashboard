...
class RecommendationBatch {
  ...
  // Google Translate API를 사용하여 영문을 한국어로 번역
  async translateToKorean(text) {
    if (!text || text.length === 0) return text;
    try {
      const url = new URL('https://translate.googleapis.com/translate_a/single');
      url.searchParams.append('client', 'gtx');
      url.searchParams.append('sl', 'en');  // 영문
      url.searchParams.append('tl', 'ko');  // 한국어
      url.searchParams.append('dt', 't');   // 번역 결과
      ...
      if (!response.ok) return text;  // 번역 실패 시 원문 반환

      const result = await response.json();
      // Google Translate API 응답 파싱: [[[번역된문장, 원문장, ...]]]
      if (result[0] && Array.isArray(result[0])) {
        return result[0].map(item => item[0]).join('');
      }
      return text;
    } catch (e) {
      console.warn(`⚠️ 번역 실패 (${text.substring(0, 20)}): ${e.message}`);
  ...
  async fetchMarketData() {
    ...
    try {
      ...
      this.koreanNames = {};
      markets.forEach(m => {
        if (m.market.startsWith('KRW-') && m.korean_name) {
          const symbol = m.market.replace('KRW-', '');
          this.koreanNames[symbol] = m.korean_name;
        }
      });

      console.log(`✅ KRW 마켓 발견: ${krwMarkets.length}개, 한글명 ${Object.keys(this.koreanNames).length}개 수집`);
  ...
  // 분석 알고리즘
  async analyzeAndRecommend(marketData, coinDescriptions = {}) {
    console.log('🔍 시장 분석 중...');
    ...
    for (const [symbol, data] of Object.entries(marketData)) {
      ...
      // COIN_DESCRIPTIONS에 없으면 CoinGecko 데이터 사용 (영문 → 한국어 번역 + 3줄 형식으로 변환)
      if (!description && coinDescriptions[symbol]) {
        const cgDesc = coinDescriptions[symbol];
        // HTML 태그 제거 및 정제
        let cleanDesc = cgDesc
          .replace(/<[^>]*>/g, '')
          .replace(/&nbsp;/g, ' ')
          .replace(/&quot;/g, '"')
          .replace(/&amp;/g, '&')
        ...
        // 영문 → 한국어 번역
        console.log(`    🌐 ${symbol} 번역 중...`);
        const koDesc = await this.translateToKorean(cleanDesc);

        // 번역된 한국어 문장 분리
        const sentences = koDesc
          .split(/[.!?」।]/)  // 한국어, 영문, 다국어 문장 구분자 포함
          .map(s => s.trim())
          .filter(s => s.length > 0);

        if (sentences.length > 0) {
          // 3줄 형식으로 변환
          const line1 = sentences[0]?.substring(0, 100) || '';
          const line2 = sentences[1]?.substring(0, 100) || sentences[0]?.substring(100, 200) || '';
          const line3 = sentences[2]?.substring(0, 100) || '';

          // 이모지와 함께 3줄 요약 생성
          const emojiList = ['🌟', '💪', '🔥', '⚡️', '💎', '🚀', '🎯', '💼', '🌐', '📈'];
          const randomEmojis = [emojiList[symbol.charCodeAt(0) % emojiList.length],
          emojiList[symbol.charCodeAt(1) % emojiList.length],
          emojiList[symbol.charCodeAt(2) % emojiList.length]];

          description = `1. ${randomEmojis[0]} ${line1}\n2. ${randomEmojis[1]} ${line2}\n3. ${randomEmojis[2]} ${line3}`.trim();
        }
      }
  ...
  saveRecommendations(recommendations) {
    ...
    // 1. utils/koreanNames.js 생성 (모든 코인의 한글명)
    console.log('📝 한글명 저장 중...');
    const koreanNamesPath = path.join(__dirname, '../utils/koreanNames.js');

    const koreanNamesContent = `// Korean name mapping for all coins
// 자동 생성: lib/batch-recommendations.js에서 매일 Upbit API로부터 생성됨

export const KOREAN_NAMES = ${JSON.stringify(this.koreanNames, null, 2)};

export default KOREAN_NAMES;
`;

    fs.writeFileSync(koreanNamesPath, koreanNamesContent);
    console.log(`✅ ${koreanNamesPath} 생성됨 (${Object.keys(this.koreanNames).length}개 코인)`);

    // 2. utils/dailyRecommendations.js 업데이트
...
`;
  ...
  async run() {
    try {
      ...
      // 3. 분석 & 추천 생성 (비동기 처리)
      const recommendations = await this.analyzeAndRecommend(marketData, coinDescriptions);
